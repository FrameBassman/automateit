namespace AutomateIt.Extensions
{
    using System;
    ...
    public class XPathBuilder
    {
        ...
        public static string Concat(string root, string relative, params object[] args)
        {
            ...
            if (relative.StartsWith(XPATH_ROOT)) relative = relative.Substring(2, relative.Length - 2);
            if (string.IsNullOrWhiteSpace(relative))
            {
                if (string.IsNullOrWhiteSpace(root)) throw new Exception("Invalid xpath: root and relative parts are empty");
                return root;
            }
            if (string.IsNullOrWhiteSpace(root)) return XPATH_ROOT + relative;
